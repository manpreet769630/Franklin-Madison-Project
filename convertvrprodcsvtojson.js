...
try {
  const csvFile = readFileSync('./cypress/fixtures/vr_prod_source.csv', 'utf8');
  const csvResults = parse(csvFile, {
    header: true,
    complete: (csvData) => csvData.data,
  }).data;
  writeFileSync(
    './cypress/fixtures/vr_source.json',
    JSON.stringify(csvResults, null, 4),
    'utf-8',
  );
  console.log('vr_prod_source.csv file is converted successfully!');
} catch (e) {
